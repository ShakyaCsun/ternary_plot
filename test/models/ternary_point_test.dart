...
void main() {
  group('TernaryPoint', () {
    group('.compareTo works correctly', () {
      test('when xyPoint is same', () {
        const pointA = TernaryPoint(a: 1, b: 0, c: 0);
        const pointB = TernaryPoint(a: 2, b: 0, c: 0);
        expect(pointA.compareTo(pointB), equals(0));
      });

      test('when y-axis for point A is less than point B', () {
        ...
        expect(pointA.compareTo(pointB), isPositive);
        expect(pointB.compareTo(pointA), isNegative);
      });

      test('when x-axis for point A is less than point B', () {
        ...
        expect(pointA.compareTo(pointB), isNegative);
        expect(pointB.compareTo(pointA), isPositive);
      });
    });
  });
  group('List of TernaryPoints', () {
    test('gets sorted top to bottom and left to right', () {
      final points = [
